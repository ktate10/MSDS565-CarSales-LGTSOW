...
### Project Overview
Before jumping into building prediction models, a thorough Exploratory Data Analysis (EDA) on this dataset was performed to get a clear picture. The goal was simple: map out the data’s overall structure, spot interesting patterns and relationships with price, and uncover every data quality issue that needs fixing before model development.
...
### Project Overview
With a preprocessed training and test sets established, this notebook focused on evaluating multiple regression algorithms to predict car prices. This was mainly done by benchmarking classical machine learning models against deep neural network architectures using rigorous 5 fold cross validation and systematically tuned hyperparameters to evaluate the models performance and identify our top performing estimators.
...
### Project Overview
After building the initial models, Random Forest feature ranking and Variance Inflation Factor (VIF) filtering were used to cut out weak or repetitive features. Afterwards, all of the models were retrained on this smaller feature set to see how much speed and performance we gained (or lost).
...
### Project Overview
In the final notebook, model interpretability and algorithmic fairness were evaluated for the top-performing estimator. SHAP was applied to inspect the model's inner workings and understand how it calculates car price predictions, followed by a fairness evaluation using Fairlearn to assess how performance holds up across preserved metadata dimensions.
